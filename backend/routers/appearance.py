...
async def update_appearance(request: Request, appearance: Appearance, token: dict = Depends(require_role("admin")), db = Depends(get_db)):
    def _update_appearance_sync():
        cur = db.cursor()
        try:
            updates = []
            params = []
    ...
    return await run_in_threadpool(_update_appearance_sync)
